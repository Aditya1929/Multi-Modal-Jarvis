...
class AppCircle:
    def __init__(self, center, radius, app_index, final_pos, is_main=False):
        ...
        self.text = 'Welcome' if is_main else f'App {app_index}'
        self.hover_time = 0
...
def create_circles():
    circles = []
    num_circles = 5
    center_x, center_y = SCREEN_SIZE[0] // 1.8, SCREEN_SIZE[1] // 1.8
    main_circle_radius = 75
    app_circle_radius = 60
    distance = 200
    ...
    while running:
        ...
        if transformed_landmarks:
            for transformed_coords in transformed_landmarks:
                ...
                screen_x = int(index_finger_tip[0] + 35)
                screen_y = int(index_finger_tip[1] + 35)
        ...
        if index_finger_pos:
            pygame.draw.circle(screen, LIGHT_BLUE, index_finger_pos, 12, 3)
